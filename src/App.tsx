...
function App() {
  ...
  const totalSpend = useMemo(() => {
    if (!orderArray) return 0
      ...
      return orderArray.reduce((total, order) => {
        const orderTotal = order.total * 100000
        return (orderTotal + total * 100000) / 100000
      }, 0)
  }, [orderArray])
    ...
    async function calculate(next: number) {
      ...
      if (next_offset >= 0) {
        for (const detail of body.data.details_list) {
          ...
          const productArray =
            detail.info_card.order_list_cards[0].product_info.item_groups.map(
              (item: any) => ({ ...item.items[0] })
            )
  ...
  return (
    <main className="bg-secondary flex flex-col justify-start gap-1 w-[min(100%,400px)] relative">
      <div className="p-2">
        ...
        {orderArray && orderArray.length > 0 && (
          <ul className="flex flex-col gap-2">
            {orderArray.map((order) => (
              <OrderCard key={crypto.randomUUID()} order={order} />
            ))}
          </ul>
        )}
      </div>

      <footer className="sticky bottom-0 left-0 bg-accent-gradient p-2 flex justify-between items-center text-slate-200">
        <div className="flex gap-2 text-xs">
          <p
            className="hover:text-blue-600"
            onClick={() =>
              chrome.tabs.create({
                url: "https://github.com/muhdyusuf/shopee-purchase-tracker",
              })
            }
          >
        ...
        <p className="flex items-center flex-wrap">
          Total :<span className="text-lg font-bold">Rm {totalSpend}</span>
        </p>
      </footer>
    </main>
  )
